...
extern "C" void atexit(fnevent proc);
...
static Cursor create_empty_cursor() {
    ...
    if (empty_cursor == None) {
        ...
        pixmap = XCreateBitmapFromData(dpy, hwnd, data, 1, 1);
        if(pixmap) {
            empty_cursor = XCreatePixmapCursor(dpy, pixmap, pixmap, &color, &color, 0, 0);
...
void sys_cursor(bool show) {
	if(!show) {
		XDefineCursor(dpy, hwnd, create_empty_cursor());
		// XFixesHideCursor(dpy, hwnd);
		XFlush(dpy);
	}
}
